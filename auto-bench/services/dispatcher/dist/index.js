...
import { TaskSpecSchema } from "@autobench/shared";
import { z } from "zod";
import fetch from "node-fetch";
var generatorResponseSchema = z.object({
...
function createOpenAIProvider(apiKey) {
  return {
    ...
    async call(spec, systemPrompt) {
      ...
      const response = await fetch2(OPENAI_URL, {
        method: "POST",
        headers: {
...
function createAnthropicProvider(apiKey) {
  return {
    ...
    async call(spec, systemPrompt) {
      ...
      const response = await fetch3(ANTHROPIC_URL, {
        method: "POST",
        headers: {
          "x-api-key": apiKey,
...
function createGoogleProvider(apiKey) {
  return {
    ...
    async call(spec, systemPrompt) {
      ...
      const response = await fetch4(endpoint, {
        method: "POST",
        headers: {
          "content-type": "application/json"
        },
...
function createDeepSeekProvider(apiKey) {
  return {
    ...
    async call(spec, systemPrompt) {
      ...
      const response = await fetch5("https://api.deepseek.com/v1/chat/completions", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
          "content-type": "application/json"
...
async function renderArtifact(rendererUrl, runtime, code) {
  const response = await fetch6(`${rendererUrl}/render`, {
    method: "POST",
...
import * as shared2 from "@autobench/shared";

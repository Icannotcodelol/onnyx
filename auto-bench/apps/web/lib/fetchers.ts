...
const extractModelInfo = (row: any) => {
  const relation = row?.models;
  const modelEntry = Array.isArray(relation) ? relation[0] : relation;
  const providerRelation = modelEntry?.model_providers;
  const providerEntry = Array.isArray(providerRelation) ? providerRelation[0] : providerRelation;

  return {
    label: modelEntry?.label ?? "Unknown",
...
export const getTaskDetail = cache(async (taskId: string) => {
  ...
  if (submissionIds.length > 0) {
    const { data: artifacts, error: artErr } = await client
      .from("artifacts")
      .select("id, submission_id, kind, storage_path, width, height, duration_ms, harness_html")
...
export const getLeaderboard = cache(async (): Promise<Rating[]> => {
  ...
  if (error) {
    console.warn("ratings_with_sparkline missing", error);
    return [];
  }
